...
CORRECT: B</pre>
        ...
        <div class="d-none" id="statusSeeds">
            <div class="aq-status-seed" data-question="Choose the correct passive voice sentence." data-answer="The cake was baked by my mother." data-type="SCQ" data-subject="Tiếng Anh" data-chapter="2" data-status="Approved"></div>
        ...
        <script>
            (function () {
                ...
                function createOptionRow(questionType, optionIndex) {
                    const row = document.createElement('div');
                    row.className = 'aq-option-row';

                    row.innerHTML =
                            '<div class="aq-option-row__left">' +
                            '   <input class="aq-option-correct" type="' + (questionType === 'SCQ' ? 'radio' : 'checkbox') + '" name="optionCorrect">' +
                            '</div>' +
                ...
                function buildQuestionBody(item) {
                    ...
                    for (let i = 0; i < 4; i++) {
                        optionsList.appendChild(createOptionRow(type, i));
                    }
                ...
                function createQuestionItem(initialPrompt) {
                    ...
                    item.innerHTML =
                            ...
                            '       <textarea class="aq-control aq-control--textarea aq-question-prompt" placeholder="Enter the question content here...">' + (initialPrompt ? escapeHtml(initialPrompt) : '') + '</textarea>' +
                            '   </div>' +
                            '</div>' +
                ...
                function refreshSummary() {
                    ...
                    if (method === 'manual') {
                        if (type === 'SCQ') scq = manualCount;
                        if (type === 'MCQ') mcq = manualCount;
                        if (type === 'Essay') essay = manualCount;
                    }
                ...
                function getStatusClass(status) {
                    ...
                    if (s === 'approved') return 'aq-status-pill aq-status-pill--approved';
                    if (s === 'rejected') return 'aq-status-pill aq-status-pill--rejected';
