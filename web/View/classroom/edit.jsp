                ...
                <form action="${ctx}/classroom/manage/edit" method="post">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <span>${requestScope.listMSG.msgClassName}</span>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <c:if test="${requestScope.totalStudent == 0}">
                                <select name="subjectId" id="subject">
                                    <c:forEach items="${requestScope.listSubject}" var="s">
                                        <option value="<c:out value='${s.id}'/>" ${classroom.subjectId == s.id ? 'selected' : ''}>${s.name}</option>
                                    </c:forEach>
                                </select>
                            </c:if>
                            <c:if test="${requestScope.totalStudent > 0}">
                                <input type="text" id="subject" name="subjectId"
                                       value="<c:out value='${subjectId}'/>"
                                       hidden>
                            ...
                            ${requestScope.totalStudent > 0 ? 'This class cannot change subject because students have already joined.' : ''}
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input name="studentJoined" value="${classroom.sum}" hidden>
                            <span class="form-hint">
                                Current enrolled: <strong>${classroom.sum}</strong>. You cannot set a limit below this number.
                            </span>  
                                <span>${requestScope.listMSG.msgMaxStudent}</span>
                        </div>

                    </div>
                    <div>
                        ${not empty requestScope.listMSG.msgNotifySuccess ? requestScope.listMSG.msgNotifySuccess : ''}
                        ${not empty requestScope.listMSG.msgNotifyError ? requestScope.listMSG.msgNotifyError : ''}
                        ${not empty requestScope.listMSG.msgNotify ? requestScope.listMSG.msgNotify : ''}
                    </div>
                    <div class="btn-row">
